...
- **文法拡張方針**: 式指向ブロック（`if/let/match` すべて式）、先行宣言可能な演算子優先度/結合性テーブル、記号/Unicode/ASCII 別名の両立、`using/handle` などリソース・エフェクト構文を第一級化。パターンはガード/ビュー/エフェクトパターンを含み、`effect` 宣言と `handle` のスコープが明示される構文を採用。

...
fn import_ffi_module() {
    let path = Path::new("samples/main.hp");
    let result = transpile_file(path).unwrap();

    // Should include external declarations from imported .ffi.hp file
...
fn ffi_file_allows_external() {
    let path = Path::new("samples/console.ffi.hp");
    let result = transpile_file(path).unwrap();
...
#[test]
fn hp_file_rejects_external() {
    // Create a temporary file with external declaration
    let temp_dir = std::env::temp_dir();
    let temp_file = temp_dir.join("test_external.hp");
    ...
    assert!(err.contains("only allowed in .ffi.hp files"));
...
fn import_file_not_found() {
    let temp_dir = std::env::temp_dir();
    let temp_file = temp_dir.join("test_import_missing.hp");
    std::fs::write(&temp_file, "import \"nonexistent.ffi.hp\"").unwrap();
